...
cameraAnglePixelH = 166/Resolution_Horizontal  #95 / 640
cameraAnglePixelV = 65/Resolution_Vertical  #65 / 640
server = "10.23.42.2"
cameraDataTable="oakData"
...
def detectionPacketCallback(packet):
    ...
    for detection in packet.detections:
        ...
        if detection.confidence >= confidence_percent:
            objectAngleY = -cameraAnglePixelV * ((((detection.bbox.ymax + detection.bbox.ymin) / 2) * Resolution_Vertical) - (Resolution_Vertical/2))
            objectAngleX = -cameraAnglePixelH * ((((detection.bbox.xmax + detection.bbox.xmin) / 2) * Resolution_Horizontal) - (Resolution_Horizontal/2))
    ...
    cameraDataTable .putStringArray("camera_" + str(cameraIdNumber) + "/data", itemPredictionReturn)
